...
class ProgressBar(Widget):
    ...
    def __init__(self, width, *args, **kwargs):
        ...
        self._baseImage = root.getGameObject().getSpriteSource('gui').get(self.PROGRESS_SPRITE)
    ...
    def _prepImage(self):
        ...
        if not hasattr(self, 'image'):
            self.image = pygame.Surface((self._width, self._baseImage.get_height()))
        ...
        while x < self._progress:
            ...
            self.image.blit(self._baseImage, (x, 0), 
                            pygame.rect.Rect((0, 0),
                                             (nextWidth, self._baseImage.get_height())))
            x += nextWidth
